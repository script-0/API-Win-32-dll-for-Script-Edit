...
/**Name:SE_ReadFile
 *Objectif: lire un fichier dans une chaine de caractere
 *Argument: fname:chemin du fichier a lire
            hwnd: HWND de la fenetre en cours
 *Return's value:  ftext : le contenu du fichier sous forme de LPSTR
**/
LPSTR SE_ReadFile(HWND hwnd,char* fname)
{
        static HANDLE hfile;
        static DWORD read;
        static DWORD fsize;
        ...
        else
        {
            ...
            {
                if(fsize < 100*1024*1024)//100 Mo
                {
                    LockFile(hfile,0,0,0,0);
                    ftext=(LPSTR)GlobalAlloc(GPTR,fsize+1);
                    if(ftext==NULL)
                    {
                        showMessageError(hwnd,"Error:Memory access denied.\nRessayer...");
                    }
                    else
                    {
                        ...
                        showMessageError(hwnd,"File to big for reading");
                    }
            }
            UnlockFile(hfile,0,0,0,0);
            CloseHandle(hfile);
 ...
 *Objectif: build pattern of Error Message Notification
 *Argument: message to Display
 *Return's value:  No return's value
**/
void showMessageError(HWND hwnd,char* message)
{
    MessageBox(hwnd,message,"We are sorry! --My Script--",MB_OK|MB_ICONERROR);
}
...
DLL_EXPORT BOOL read_drop_file(HWND hwnd ,BOOL* Setfile,char* fname)
{
    BOOL result=FALSE;
    HANDLE hfile=NULL;
    char* ftext=NULL; //0x0D
    static DWORD read;
    static DWORD fsize;
    //setlocale(LC_ALL,".1252");//.iso-8859-1
    ...
    {           if(fileExist(fname))
                ...
                CloseHandle(hfile);
                if(fsize>1024*1024*8)//8 Mo
                {
                   MessageBox(NULL,TEXT("\tBackup file failed. File too big!\nIt will be impossible to restore initial file after any modification"),TEXT("Note"),MB_OK|MB_ICONINFORMATION);
                }
                else{
...
/**Name: remove_from_recent_file
 *Objectif: retirer le path pris en argument dans la liste des fichiers ouverts et retourne l'index du path suivant apres l'avoir selectionne.
            Retourne -1 si la liste est vide.Dans le cas ou le path==null, il supprime le path selectionne dans le list box
 *Entree: le HWND de la fenetre
          le path en question
 *Sortie: index en question
 */
 DLL_EXPORT int remove_from_recent_file(HWND hwnd_window, const char* path)
 {
    HWND hwnd=GetDlgItem(hwnd_window,IDM_CAPTION);
    int nItem=0, nTotal=0,result=-1;
    if(path == NULL)
    {
        nItem = SendMessage(hwnd,CB_GETCURSEL, 0, 0);
        if (nItem == CB_ERR)
        {
             showMessageError(hwnd,"Error on remove_from_recent_file: case where path is null. nItem return LB_ERR");
            return result;
        }
        nTotal = SendMessage(hwnd, CB_DELETESTRING, nItem, 0);
        ...
        {
            SendMessage(hwnd,CB_SETCURSEL, nTotal-1, 0);
            result = nTotal-1;
        }
    }
